...
1. **Backend unit tests**: `.venv/bin/pytest tests/ -v` (1095 tests; all of them
...
2. **Frontend unit tests**: `cd web && bun run test` (182 tests)
...
4. **E2E tests**: `cd web && bun run test:e2e` (78 tests, no skips, ~4 min, Playwright
   ...
   `bun run test:e2e:llm` (36 real-LLM tests, ~3 min — chat, /schedule,
