...
2. **Frontend unit tests**: `cd web && bun run test` (172 tests)
3. **TypeScript check**: `cd web && npx tsc --noEmit`
4. **E2E tests**: `cd web && bun run test:e2e` (76 tests, no skips, ~4 min, Playwright
   auto-starts servers). Split into two buckets for dev iteration —
   `bun run test:e2e:fast` (41 pure-UI tests, ~33 s — login / sessions /
