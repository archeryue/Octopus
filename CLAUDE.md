...
1. **Backend unit tests**: `.venv/bin/pytest tests/ -v` (743 tests; the
...
- `server/mcp_servers/ask_agent.py` — Stdio MCP server exposing the four delegation tools to the model (`ask_agent`, `cancel_agent_task`, `list_agent_tasks`, `answer_agent_question`). Same `OCTOPUS_API_BASE` / `OCTOPUS_SESSION_ID` env-injection pattern as the bg + ask built-ins; thin HTTP shim to the `/api/sessions/{sid}/delegations` routes. Added to the default per-agent MCP set; the migration backfills it onto every pre-existing agent row.
