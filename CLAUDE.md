...
1. **Backend unit tests**: `.venv/bin/pytest tests/ -v` (1044 tests; all of them
...
2. **Frontend unit tests**: `cd web && bun run test` (163 tests)
3. **TypeScript check**: `cd web && npx tsc --noEmit`
4. **E2E tests**: `cd web && bun run test:e2e` (74 tests, no skips, ~3 min, Playwright
   auto-starts servers). Split into two buckets for dev iteration —
   `bun run test:e2e:fast` (39 pure-UI tests, ~30 s — login / sessions /
