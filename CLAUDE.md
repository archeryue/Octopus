...
1. **Backend unit tests**: `.venv/bin/pytest tests/ -v` (984 tests; all of them
...
2. **Frontend unit tests**: `cd web && bun run test` (139 tests)
3. **TypeScript check**: `cd web && npx tsc --noEmit`
4. **E2E tests**: `cd web && bun run test:e2e` (71 tests, no skips, ~3 min, Playwright
   auto-starts servers). Split into two buckets for dev iteration —
   `bun run test:e2e:fast` (36 pure-UI tests, ~20 s — login / sessions /
   dialogs / sidebar / virtualized chat / attachments / etc.) and
   `bun run test:e2e:llm` (34 real-LLM tests, ~2.5 min — chat, /schedule,
