...
1. **Backend unit tests**: `.venv/bin/pytest tests/ -v` (1072 tests; all of them
...
2. **Frontend unit tests**: `cd web && bun run test` (176 tests)
3. **TypeScript check**: `cd web && npx tsc --noEmit`
4. **E2E tests**: `cd web && bun run test:e2e` (77 tests, no skips, ~4 min, Playwright
   auto-starts servers). Split into two buckets for dev iteration —
   `bun run test:e2e:fast` (42 pure-UI tests, ~35 s — login / sessions /
   dialogs / sidebar / virtualized chat / attachments / etc.) and
   `bun run test:e2e:llm` (35 real-LLM tests, ~2.5 min — chat, /schedule,
