...
1. **Backend unit tests**: `.venv/bin/pytest tests/ -v` (973 tests; all of them
...
2. **Frontend unit tests**: `cd web && bun run test` (133 tests)
3. **TypeScript check**: `cd web && npx tsc --noEmit`
4. **E2E tests**: `cd web && bun run test:e2e` (70 tests, no skips, ~3 min, Playwright
   ...
   `bun run test:e2e:llm` (33 real-LLM tests, ~2.5 min — chat, /schedule,
