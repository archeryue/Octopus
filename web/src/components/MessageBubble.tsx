      ...
      return message.role === "user" ? (
        <div className="msg msg-user flex justify-end">
          <div className="max-w-[85%] space-y-1">
            ...
            <div className="msg-content inline-block rounded-lg bg-primary px-3 py-2 text-sm text-primary-foreground whitespace-pre-wrap break-words">
              {message.content}
            </div>
          </div>
        </div>
        ...
        <div className="msg msg-assistant space-y-1">
          ...
          <div className="msg-content markdown rounded-lg border border-border bg-card px-3 py-2 text-sm leading-relaxed">
      ...
      return (
        <div className="msg msg-user flex justify-end">
          <div className="max-w-[85%] space-y-1">
            ...
            <div className="msg-content msg-question-answer inline-block rounded-lg bg-primary/80 px-3 py-2 text-sm text-primary-foreground italic whitespace-pre-wrap break-words">
              {message.content}
            </div>
          </div>
        </div>
    ...
    case "error":
      return (
        <div className="msg msg-error space-y-1">
          ...
          <div className="msg-content rounded-lg border border-destructive/40 bg-destructive/10 px-3 py-2 text-sm text-destructive whitespace-pre-wrap break-words">
            {message.content}
          </div>
        </div>
      );
...
function ToolUseBlock({ message }: { message: Message }) {
  ...
  return (
    <div className="msg msg-tool rounded-lg border border-border bg-card overflow-hidden">
      ...
      {expanded && (
        <pre className="tool-detail border-t border-border bg-muted/40 px-3 py-2 text-xs font-mono text-foreground overflow-x-auto whitespace-pre-wrap break-words max-h-80 overflow-y-auto">
          {inputStr}
        </pre>
      )}
    </div>
...
function ToolResultBlock({ message }: { message: Message }) {
  ...
  return (
    ...
    >
      ...
      {expanded && (
        <pre
          className={`tool-detail border-t px-3 py-2 text-xs font-mono whitespace-pre-wrap break-words max-h-80 overflow-y-auto ${
            errored
