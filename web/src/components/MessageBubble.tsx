...
function ToolUseBlock({ message }: { message: Message }) {
  ...
  return (
    <div className="msg msg-tool rounded-lg border border-border bg-card overflow-hidden">
      ...
      {expanded && (
        <pre className="tool-detail border-t border-border bg-muted/40 px-4 py-3 text-xs font-mono text-foreground overflow-x-auto whitespace-pre-wrap break-words max-h-80 overflow-y-auto">
          {inputStr}
        </pre>
      )}
    </div>
...
function ToolResultBlock({ message }: { message: Message }) {
  ...
  return (
    ...
    >
      ...
      {expanded && (
        <pre
          className={`tool-detail border-t px-4 py-3 text-xs font-mono whitespace-pre-wrap break-words max-h-80 overflow-y-auto ${
            errored
