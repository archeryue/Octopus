...
/** Octopus mark — same SVG as the favicon, inlined so it scales cleanly.
 *
 * It paints itself in `--color-logo` (the fixed brand navy) rather than
 * inheriting `currentColor` from whatever it sits in: the mark is artwork,
 * not chrome, so it must not move when the interface palette does. Pass a
 * `className` with a text colour only if you deliberately want it tinted. */
