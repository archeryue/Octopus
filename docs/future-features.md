...
*(item #4 closed — see `server/backends/claude_code.py`
`_send_control_response_with_content`. We own the CLI subprocess and
drive its control protocol directly, so the deny-channel content
return is just the API word for what we want, not a workaround for a
missing SDK feature. No MCP server needed.)*
